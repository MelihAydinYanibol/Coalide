...
When a daily time limit is set (`DAILY_COALIDE_TIME_LIMIT`), the *Günlük Süre
Bütçesi* card and the *Özet* card on **Genel Bakış** also show the same "at this
pace" estimate the child sees on the app's main menu: how many more questions
the remaining time is good for, at today's average seconds per question (before
the day's first timed answer it falls back to the all-time average and says so).
